...
import com.wanhao.wms.bean.TransferOutDetailsBean;
import com.wanhao.wms.bean.TransferOutOrderBean;
...
import com.wanhao.wms.ui.function.TransferOutDocDetailsActivity;
import com.wanhao.wms.ui.function.TransferSnListActivity;
 ...
 * @author ql
 */
@BindPresenter(titleRes = R.string.transfer_in)
public class TransferInOperatePresenter extends DefaultGoodsListPresenter {
    public static final String DOC = "doc";
    private ArrayList<TransferOutDetailsBean> mGoodsAll;
    private TransferOutDetailsBean mToChangeGoods;
    private String mTargetRack;
    private CommGoodsComputer mGoodsComputer = new CommGoodsComputer();

    public static void putDoc(TransferOutOrderBean iDoc, Bundle bundle) {
        iDoc.setLabels(null);
    ...
    private void addGoods(DecodeBean data, ComGoods goods, double canAddQty, String targetRack) {
        TransferOutDetailsBean saveGoods = isSaveGoods(data, targetRack);

        if (saveGoods == null) {
            TransferOutDetailsBean clone = (TransferOutDetailsBean) ((TransferOutDetailsBean) goods.getListKey().get(0)).clone();
    ...
    private TransferOutDetailsBean isSaveGoods(IGoodsDecode data, String rack) {
        //是否已经存在显示货品货品
        for (IDoc iDoc : mGoodsList) {
            TransferOutDetailsBean d = (TransferOutDetailsBean) iDoc;
    ...
    private TransferOutOrderBean mDocOrder;
    ...
    public void init(Bundle bundle) {
        ...
        mDocOrder = JsonUtils.fromJson(string, TransferOutOrderBean.class);
    ...
    private void loadDocGoods() {
        ...
        OkHttpHeader.post(UrlApi.transfer_in_order_details, mParams, new BaseResultCallback() {
            ...
            protected void onResult(BaseResult resultObj, int id) {
                mGoodsAll = resultObj.getList(TransferOutDetailsBean.class);
    ...
    public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
        TransferOutDetailsBean iDoc = (TransferOutDetailsBean) mGoodsList.get(position);
        if (iDoc.isAutoSn()) {
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, final int position) {
        TransferOutDetailsBean pd = (TransferOutDetailsBean) mGoodsList.get(position);
    ...
    public void actionSubmit() {
        ...
        for (IDoc iDoc : mGoodsList) {
            TransferOutDetailsBean pd = (TransferOutDetailsBean) iDoc;
            if (pd.isSerial()) {
        ...
        Ok:
        for (IDoc iDoc : mGoodsList) {
            TransferOutDetailsBean pds = (TransferOutDetailsBean) iDoc;
            TransferOutDetailsBean pd = (TransferOutDetailsBean) pds.clone();
            if (pd.isSerial()) {
                if (pd.getNowQty() < pd.getOpQty()) {
                    iDialog.displayMessageDialog("序列号必须全部提交，否则不可以进行提交操作!!!");
                    iDialog.cancelLoadingDialog();
            ...
            for (Object o : goodsKey) {
                TransferOutDetailsBean d = (TransferOutDetailsBean) o;
                double opQty = d.getOpQty();

                Map<String, Object> map = new HashMap<>();
                map.put("id", d.getId());
                map.put("lineNo", d.getLineNo());
                map.put("transCode", d.getTransCode());
    ...
    public void handleClickSeeSnList(IDoc d) {
        mToChangeGoods = (TransferOutDetailsBean) d;
        Bundle bundle = new Bundle();
        TransferSnListActivity.put(mToChangeGoods.getSnList(), mToChangeGoods, bundle);
        iToAction.startActivity(TransferSnListActivity.class, bundle);
    }
    ...
    public void changeGoods(TransferOutDetailsBean pd) {
